...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
  ...
  <xsl:template match="page">
    <copy file="${{src.xml.dir}}/build-master-style.xsl" toFile="${{build.tmpgen.dir}}{@datafile}{@tempin}.xsl" filtering="true" overwrite="true">
      <filterset>
        <filter token="page.template" value="../..{../../@basedir}{@template}"/>
        <filter token="page.layout" value="../..{../../@basedir}{@layout}"/>
        <filter token="page.renderer" value="../..{../../@basedir}{@renderer}"/>
      </filterset>
    </copy>

    <style force="yes">
      <xsl:attribute name="style"><xsl:text>${build.tmpgen.dir}</xsl:text><xsl:value-of select="@datafile"/><xsl:value-of select="@tempin"/><xsl:text>.xsl</xsl:text></xsl:attribute>
      <xsl:choose>
        <xsl:when test="@tempin != ''">
          <xsl:attribute name="in"><xsl:text>${build.tmpgen.dir}</xsl:text><xsl:value-of select="@tempin"/></xsl:attribute>
        </xsl:when>
        <xsl:otherwise>
